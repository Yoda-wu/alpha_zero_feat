...
class PolicyValueNetNumpy():
    ...
    def policy_value_fn(self, board):
        ...
        for i in [1, 2, 3]:
            X = relu(conv_forward(X, self.params['conv'+str(i)+'.weight'],
                            self.params['conv'+str(i)+'.bias']))
        # policy head
        # X_p = relu(conv_forward(X, self.params[6], self.params[7], padding=0))
        # X_p = fc_forward(X_p.flatten(), self.params[8], self.params[9])
        X_p = relu(conv_forward(X, self.params['act_conv1.weight'],
                            self.params['act_conv1.bias'], padding=0))
        X_p = fc_forward(X_p.flatten(), self.params['act_fc1.weight'],
                            self.params['act_fc1.bias'])
        ...
        X_v = relu(conv_forward(X, self.params['val_conv1.weight'],
                            self.params['val_conv1.bias'], padding=0))
        X_v = relu(fc_forward(X_v.flatten(), self.params['val_fc1.weight'],
                            self.params['val_fc1.bias']))
        value = np.tanh(fc_forward(X_v, self.params['val_fc2.weight'],
                            self.params['val_fc2.bias']))
